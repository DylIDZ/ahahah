...
function Library:Loader(config)
    config = config or {}
    local word = config.Title or "ASTRALUX"
    ...
    for i = 1, 20 do
        ...
        particleGlow.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.fromRGB(math.min(math.floor(accentColor.R*255) + 50, 255), math.min(math.floor(accentColor.G*255) + 50, 255), math.min(math.floor(accentColor.B*255) + 50, 255))),
        ...
        task.spawn(function()
            while particle.Parent do
                ...
                TweenService:Create(particle, TweenInfo.new(randomDuration, TweenInfo.new().Time, Enum.EasingStyle.Linear), {
                    Position = UDim2.new(newX, 0, newY, 0),
                    BackgroundTransparency = math.random(70, 90)/100
                }):Play()
                
                task.wait(randomDuration)
            end
        end)
    end
    ...
    for i = 1, 8 do
        ...
        lineGradient.Color = ColorSequence.new({
            ...
            ColorSequenceKeypoint.new(0.5, Color3.fromRGB(math.min(math.floor(accentColor.R*255) + 50, 255), math.min(math.floor(accentColor.G*255) + 50, 255), math.min(math.floor(accentColor.B*255) + 50, 255))),
            ColorSequenceKeypoint.new(1, accentColor)
        })
    ...
    for i = 1, #word do
        ...
        label.TextStrokeColor3 = accentColor
        ...
        gradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.fromRGB(math.min(math.floor(accentColor.R*255) + 50, 255), math.min(math.floor(accentColor.G*255) + 50, 255), math.min(math.floor(accentColor.B*255) + 50, 255))),
            ColorSequenceKeypoint.new(0.5, Color3.fromRGB(math.min(math.floor(accentColor.R*255) + 100, 255), math.min(math.floor(accentColor.G*255) + 100, 255), math.min(math.floor(accentColor.B*255) + 100, 255))),
            ColorSequenceKeypoint.new(1, accentColor)
        })
...
function Library:Window(p)
	...
	function Tabs:Tab(p)
		...
		function Func:Code(p)
			...
			-- Copy button UI elements creation removed
			...
			-- Copy button UI elements removed
